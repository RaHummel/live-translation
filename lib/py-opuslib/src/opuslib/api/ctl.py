...
get_final_range = get(opuslib.GET_FINAL_RANGE_REQUEST, ctypes.c_uint)
...
get_complexity = get(opuslib.GET_COMPLEXITY_REQUEST, ctypes.c_int)
...
get_vbr_constraint = get(opuslib.GET_VBR_CONSTRAINT_REQUEST, ctypes.c_int)

# Configures mono/stereo forcing in the encoder
set_force_channels = ctl_set(opuslib.SET_FORCE_CHANNELS_REQUEST)

# Gets the encoder's forced channel configuration
get_force_channels = get(opuslib.GET_FORCE_CHANNELS_REQUEST, ctypes.c_int)

# Configures the maximum bandpass that the encoder will select automatically
set_max_bandwidth = ctl_set(opuslib.SET_MAX_BANDWIDTH_REQUEST)

# Gets the encoder's configured maximum allowed bandpass
get_max_bandwidth = get(opuslib.GET_MAX_BANDWIDTH_REQUEST, ctypes.c_int)
...
get_application = get(opuslib.GET_APPLICATION_REQUEST, ctypes.c_int)

# Gets the sampling rate the encoder or decoder was initialized with
get_sample_rate = get(opuslib.GET_SAMPLE_RATE_REQUEST, ctypes.c_int)
...
get_inband_fec = get(opuslib.GET_INBAND_FEC_REQUEST, ctypes.c_int)

# Configures the encoder's expected packet loss percentage
set_packet_loss_perc = ctl_set(opuslib.SET_PACKET_LOSS_PERC_REQUEST)

# Gets the encoder's configured packet loss percentage
get_packet_loss_perc = get(opuslib.GET_PACKET_LOSS_PERC_REQUEST, ctypes.c_int)
...
get_demixing_matrix_size = get(opuslib.OPUS_PROJECTION_GET_DEMIXING_MATRIX_SIZE_REQUEST, ctypes.c_int)
