...
def create_state(
    fs: int, channels: int, mapping_family: int, streams: int, coupled_streams: int, application: int
) -> ctypes.Structure:
    ...
    encoder_state = libopus_create(
        fs, channels, mapping_family, streams_, coupled_streams_, application, ctypes.byref(result_code)
    )
...
# FIXME: Remove typing.Any once we have a stub for ctypes
def encode(
    encoder_state: ctypes.Structure, pcm_data: bytes, frame_size: int, max_data_bytes: int
    ...
    result = libopus_projection_encode(encoder_state, pcm_pointer, frame_size, opus_data, max_data_bytes)

    if result < 0:
        raise opuslib.OpusError('Opus Encoder returned result="{}"'.format(result))
...
def encode_float(
    ...
    result = libopus_projection_encode_float(encoder_state, pcm_pointer, frame_size, opus_data, max_data_bytes)
...
def get_demixing_matrix(encoder_state: ctypes.Structure, matrix_size) -> typing.Union[int, typing.Any]:
    matrix = (ctypes.c_ubyte * matrix_size)()
    pmatrix = ctypes.cast(matrix, opuslib.api.c_ubyte_pointer)
    ret = libopus_ctl(encoder_state, opuslib.OPUS_PROJECTION_GET_DEMIXING_MATRIX_REQUEST, pmatrix, matrix_size)
    if ret != opuslib.OK:
...
# FIXME: Remove typing.Any once we have a stub for ctypes
def encoder_ctl(encoder_state: ctypes.Structure, request, value=None) -> typing.Union[int, typing.Any]:
