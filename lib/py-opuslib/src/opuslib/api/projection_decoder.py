...
def create_state(fs: int, channels: int, streams: int, coupled_streams: int, demixing_matrix: list) -> ctypes.Structure:
    """
    ...
    decoder_state = libopus_create(
        fs, channels, streams, coupled_streams, _udemixing_matrix, demixing_matrix_size, ctypes.byref(result_code)
    )
