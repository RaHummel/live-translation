...
class EncoderTest(unittest.TestCase):
    ...
    def _test_unsupported_sample_rates(self):
        for cxx in range(0, 4):
            for ixx in range(-7, 96000 + 1):
                ...
                try:
                    opuslib.api.encoder.create_state(fsx, cxx, opuslib.APPLICATION_VOIP)
    ...
    def test_force_channels(self):
        self.check_setget(
            opuslib.api.ctl.set_force_channels, opuslib.api.ctl.get_force_channels, (-1, 3), (1, opuslib.AUTO)
        )
    ...
    def test_bandwidth(self):
        ...
        ixx = -2
        self.assertRaises(
            opuslib.OpusError, lambda: opuslib.api.encoder.encoder_ctl(enc, opuslib.api.ctl.set_bandwidth, ixx)
        )

        ix1 = opuslib.BANDWIDTH_FULLBAND + 1
        self.assertRaises(
            opuslib.OpusError, lambda: opuslib.api.encoder.encoder_ctl(enc, opuslib.api.ctl.set_bandwidth, ix1)
        )

        ix2 = opuslib.BANDWIDTH_NARROWBAND
        opuslib.api.encoder.encoder_ctl(enc, opuslib.api.ctl.set_bandwidth, ix2)

        ix3 = opuslib.BANDWIDTH_FULLBAND
        opuslib.api.encoder.encoder_ctl(enc, opuslib.api.ctl.set_bandwidth, ix3)

        ix4 = opuslib.BANDWIDTH_WIDEBAND
        opuslib.api.encoder.encoder_ctl(enc, opuslib.api.ctl.set_bandwidth, ix4)

        ix5 = opuslib.BANDWIDTH_MEDIUMBAND
        opuslib.api.encoder.encoder_ctl(enc, opuslib.api.ctl.set_bandwidth, ix5)
    ...
    def test_max_bandwidth(self):
        ...
        i = -2
        self.assertRaises(
            opuslib.OpusError, lambda: opuslib.api.encoder.encoder_ctl(enc, opuslib.api.ctl.set_max_bandwidth, i)
        )
    ...
    def test_inband_fec(self):
        self.check_setget(opuslib.api.ctl.set_inband_fec, opuslib.api.ctl.get_inband_fec, (-1, 3), (1, 0))

    def test_packet_loss_perc(self):
        self.check_setget(
            opuslib.api.ctl.set_packet_loss_perc, opuslib.api.ctl.get_packet_loss_perc, (-1, 101), (100, 0)
        )
    ...
    def check_setget(self, v_set, v_get, bad, good):
        ...
        for value in bad:
            with self.assertRaises(opuslib.OpusError):
                opuslib.api.encoder.encoder_ctl(enc, v_set, value)
