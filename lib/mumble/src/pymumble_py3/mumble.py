...
from . import blobs, callbacks, channels, commands, mumble_pb2, soundoutput, tools, users
...
class Mumble(threading.Thread):
    ...
    def init_connection(self):
        ...
        self.sound_output = soundoutput.SoundOutput(
            self, PYMUMBLE_AUDIO_PER_PACKET, self.bandwidth, stereo=self.stereo, opus_profile=self.__opus_profile
        ...
        while True:
            ...
            if self.connect() >= PYMUMBLE_CONN_STATE_FAILED:  # some error occurred, exit here
                ...
                if not self.reconnect or not self.parent_thread.is_alive():
                    self.Log.debug('Connection rejected or failed, stopping thread.')
                    break
                else:
    ...
    def loop(self):
        ...
        while (
            self.connected not in (PYMUMBLE_CONN_STATE_NOT_CONNECTED, PYMUMBLE_CONN_STATE_FAILED)
    ...
    def ping_response(self, mess):
        ...
        try:
            self.ping_stats['var'] = (
                self.ping_stats['var'] + pow(old_avg - new_avg, 2) + (1 / nb) * pow(ping - new_avg, 2)
            )
