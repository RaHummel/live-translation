...
class SoundQueue:
    ...
    def __init__(self, mumble_object):
        ...
        self.decoders = {PYMUMBLE_AUDIO_TYPE_OPUS: opuslib.Decoder(PYMUMBLE_SAMPLERATE, 1)}
...
class SoundChunk:
    ...
    def extract_sound(self, duration):
        ...
        result = SoundChunk(self.pcm[:size], self.sequence, size, self.time, self.type, self.target, self.timestamp)
